...
public class SettingSpecific {
	...
	public static ModelMain generateModel() {
		...
		model.addSubModel("config.property.display.horoverlay.fragments", new ModelSimpleTextured(StellarSkyResources.fillfragment));
		model.addSubModel("config.property.display.eqoverlay.fragments", new ModelSimpleTextured(StellarSkyResources.fillfragment));
		model.addSubModel("config.property.display.ecoverlay.fragments", new ModelSimpleTextured(StellarSkyResources.fillfragment));
	...
	public static class ModelOnOff implements IRenderModel {
		...
		public void renderModel(String info, IRectangleBound totalBound, IRectangleBound clipBound,
			...
			if(info.equals("true")) {
				model.renderModel(info, totalBound, clipBound, tessellator, textureManager, colors);
			} else {
				colors[0] *= 0.7f;
				colors[1] *= 0.7f;
				colors[2] *= 0.7f;
	...
	public static class ModelTF implements IRenderModel {
		...
		public void renderModel(String info, IRectangleBound totalBound, IRectangleBound clipBound,
				Tessellator tessellator, TextureManager textureManager, float[] colors) {
			if(info.equals("true")) {
				trueModel.renderModel(info, totalBound, clipBound, tessellator, textureManager, colors);
			} else {
				falseModel.renderModel(info, totalBound, clipBound, tessellator, textureManager, colors);
			}
		}
	}
