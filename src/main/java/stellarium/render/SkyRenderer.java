...
public class SkyRenderer extends IRenderHandler {
    ...
    private void renderSkyEnd(float partialTicks)
    {
		...
		this.renderCelestial(new StellarRenderInfo(mc, tessellator, vertexbuffer, 0.0f, 1.0f, partialTicks));
	...
	public void render(float partialTicks, WorldClient theWorld, Minecraft mc)
	{
		if (this.mc.theWorld.provider.getDimensionType().getId() == 1)
        {
			...
			this.renderCelestial(new StellarRenderInfo(mc, tessellator, vertexbuffer, bglight, weathereff, partialTicks));
